...
export default function CheckoutPage() {
  ...
  const selectedShipping = shippingMethods.find(m => m.id === formData.shipping_method)
      ...
      <div className="flex items-center justify-center mb-8">
        {['Địa chỉ', 'Thanh toán', 'Xác nhận'].map((label, index) => (
          <div key={index} className="flex items-center">
            ...
            {index < 2 && <div className={`w-16 sm:w-24 h-1 mx-4 ${step > index + 1 ? 'bg-green-500' : 'bg-gray-200'}`} />}
          </div>
        ))}
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-xl p-6 shadow-sm">
            {step === 1 && (
              <div className="space-y-4">
                <h2 className="font-semibold text-gray-800 text-lg mb-4">Thông tin giao hàng</h2>
                ...
                <div>
                  <label className="block text-sm text-gray-600 mb-2">Phương thức vận chuyển</label>
                  <div className="space-y-2">
                    {shippingMethods.map((method) => (
                      <label 
                        key={method.id}
                        className={`flex items-center justify-between p-4 border rounded-lg cursor-pointer transition-colors ${
                          formData.shipping_method === method.id ? 'border-primary-500 bg-primary-50' : 'border-gray-200 hover:border-gray-300'
                        }`}
                      >
                        ...
                        <span className={method.price === 0 ? 'text-green-600' : 'font-medium'}>
            ...
            {step === 2 && (
              <div className="space-y-4">
                <h2 className="font-semibold text-gray-800 text-lg mb-4">Phương thức thanh toán</h2>
                  ...
                  {paymentMethods.map((method) => (
                    <label 
                      ...
                      className={`flex items-center p-4 border rounded-lg cursor-pointer transition-colors ${
                        formData.payment_method === method.id ? 'border-primary-500 bg-primary-50' : 'border-gray-200 hover:border-gray-300'
                      }`}
                    >
                      <input
                        type="radio"
            ...
            {step === 3 && (
              <div className="space-y-6">
                <h2 className="font-semibold text-gray-800 text-lg">Xác nhận đơn hàng</h2>
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-gray-50 rounded-lg">
                    <h3 className="font-medium text-gray-800 mb-2">Phương thức vận chuyển</h3>
                  ...
                  <div className="border-t pt-4">
                    ...
                    {cart.items.map((item) => (
                      <div key={item.book_id} className="flex justify-between py-2">
            ...
            <div className="flex justify-between mt-8 pt-6 border-t">
              ...
              >
                {loading ? 'Đang xử lý...' : step === 3 ? 'Đặt hàng' : 'Tiếp tục'}
              </button>
            </div>
          </form>
        </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl p-6 shadow-sm sticky top-24">
            <h2 className="font-semibold text-gray-800 text-lg mb-4">Đơn hàng</h2>
              ...
              {cart.items.map((item) => (
                <div key={item.book_id} className="flex gap-3">
                  <div className="w-16 h-20 bg-gray-100 rounded flex-shrink-0">
                    {item.book?.image_url ? (
                      <img src={item.book.image_url} alt="" className="w-full h-full object-cover rounded" />
                    ) : (
            ...
            <div className="space-y-2 border-b pb-4 mb-4">
              ...
              <div className="flex justify-between text-gray-600">
                ...
                <span className={selectedShipping?.price === 0 ? 'text-green-600' : ''}>
                  {selectedShipping?.price === 0 ? 'Miễn phí' : formatPrice(selectedShipping?.price || 0)}
                </span>
              </div>
            </div>
