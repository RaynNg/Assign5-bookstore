...
export default function BookDetailPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (bookRes.data.catalog_id) {
          try {
            const catalogRes = await catalogService.getById(bookRes.data.catalog_id)
        ...
        try {
          ...
          setComments(commentsRes.data.reviews || commentsRes.data.results || commentsRes.data || [])
        } catch {}
      } catch (error) {
  ...
  if (!book) return <div className="container mx-auto px-4 py-8">Không tìm thấy sách</div>
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <nav className="text-sm text-gray-500 mb-6">
        ...
        {catalog && (
          <>
            <span className="mx-2">/</span>
            <Link to={`/books?catalog=${catalog.id}`} className="hover:text-primary-600">
              {catalog.name}
            </Link>
          </>
        )}
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div>
            ...
            <div className="border-t pt-6 space-y-2 text-sm text-gray-500">
              <p><span className="font-medium text-gray-700">ISBN:</span> {book.isbn}</p>
              {catalog && (
                <p><span className="font-medium text-gray-700">Danh mục:</span> {catalog.name}</p>
              )}
            </div>
          </div>
        </div>
            ...
            <div className="space-y-6">
              ...
              {isAuthenticated && (
                <form onSubmit={handleSubmitComment} className="border-b pb-6">
                  <h3 className="font-medium text-gray-800 mb-4">Viết đánh giá của bạn</h3>
                  <div className="mb-4">
                    ...
                    <div className="flex gap-1">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <button
                          key={star}
                          type="button"
                          onClick={() => setNewComment(c => ({ ...c, rating: star }))}
                          className="text-2xl"
                        >
                          <FiStar 
                            className={`w-8 h-8 ${
